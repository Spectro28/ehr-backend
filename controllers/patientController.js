...
const empresasValidas = ['CARDIOVASC', 'INVITROMED', 'Centro de Especialidades Médicas Prado Gómez', 'COIDHEX']; // Lista de empresas válidas
...
exports.getPacientesSinIdentificacion = async (req, res) => {
  try {
    const { count, rows } = await Patient.findAndCountAll({
      ...
      attributes: ['id', 'cedula', 'primer_nombre', 'apellido_paterno', 
                   'apellido_materno', 'tipo_identificacion'], // Solo lo necesario
      limit: 50, // Un límite razonable para alertas
      order: [['createdAt', 'DESC']]
    });

    res.json({ total: count, patients: rows });
  } catch (error) {
...
exports.getPatients = async (req, res) => {
  try {
    ...
    const { count, rows } = await Patient.findAndCountAll({
      attributes: { 
        exclude: ['nombre_parroquia', 'nombre_canton', 'nombre_provincia']
      },
      where,
      limit: parsedLimit,   // ✅ Cambiado
      offset: offset,       // ✅ Cambiado
    ...
    const response = {
      total: count,
      pages: Math.ceil(count / parsedLimit),  // ✅ Cambiado
      currentPage: parsedPage,                // ✅ Cambiado
      patients: rows
    };
    ...
    // - console.log(`Resultados: ${count} pacientes...`)
    // - console.log('Respuesta final:', JSON.stringify(response)) <- el más peligroso
    
  } catch (error) {
    // ✅ Solo dejamos el console.error para errores reales
